	...
	private $error = array();
  	...
  	public function insert() {
    	...
    	if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validateForm()) {
			...
			$this->redirect(HTTPS_SERVER . 'index.php?route=catalog/promotags&token=' . $this->session->data['token'] . $url);
    	}
  	...
  	private function getList() {
		...
		if (isset($this->request->get['order'])) {
  		...
  		$this->document->breadcrumbs = array();
   		...
   		$this->document->breadcrumbs[] = array(
       		'href'      => HTTPS_SERVER . 'index.php?route=catalog/promotags&token=' . $this->session->data['token'] . $url,
       		'text'      => $this->language->get('heading_title'),
      		'separator' => ' :: '
   		);

		$this->data['insert'] = HTTPS_SERVER . 'index.php?route=catalog/promotags/insert&token=' . $this->session->data['token'] . $url;
		$this->data['delete'] = HTTPS_SERVER . 'index.php?route=catalog/promotags/delete&token=' . $this->session->data['token'] . $url;
		...
		foreach ($results as $result) {
			...
			$action[] = array(
				...
				'href' => HTTPS_SERVER . 'index.php?route=catalog/promotags/update&token=' . $this->session->data['token'] . '&promo_tags_id=' . $result['promo_tags_id'] . $url
			);
		...
		$this->data['sort_promo_text'] = HTTPS_SERVER . 'index.php?route=catalog/promotags&token=' . $this->session->data['token'] . '&sort=pt.promo_text' . $url;
		$this->data['sort_sort_order'] = HTTPS_SERVER . 'index.php?route=catalog/promotags&token=' . $this->session->data['token'] . '&sort=pt.sort_order' . $url;
		...
		$pagination->url = HTTPS_SERVER . 'index.php?route=catalog/promotags&token=' . $this->session->data['token'] . $url . '&page={page}';
  	...
  	private function getForm() {
		...
		if (!isset($this->request->get['promo_tags_id'])) {
			$this->data['action'] = HTTPS_SERVER . 'index.php?route=catalog/promotags/insert&token=' . $this->session->data['token'] . $url;
		} else {
			$this->data['action'] = HTTPS_SERVER . 'index.php?route=catalog/promotags/update&token=' . $this->session->data['token'] . '&promo_tags_id=' . $this->request->get['promo_tags_id'] . $url;
		}

		$this->data['cancel'] = HTTPS_SERVER . 'index.php?route=catalog/promotags&token=' . $this->session->data['token'] . $url;
		...
		if (isset($promotags_info) && $promotags_info['image'] && file_exists(DIR_IMAGE . $promotags_info['image'])) {
			$this->data['preview'] = $this->model_tool_image->resize($promotags_info['image'], 100, 100);
		} else {
			$this->data['preview'] = $this->model_tool_image->resize('no_image.jpg', 100, 100);
		}
		
		if (isset($promotags_info) && $promotags_info['pimage'] && file_exists(DIR_IMAGE . $promotags_info['pimage'])) {
			$this->data['preview1'] = $this->model_tool_image->resize($promotags_info['pimage'], 100, 100);
		} else {
  	...
  	private function validateDelete() {
		...
		foreach ($this->request->post['selected'] as $promotags_id) {
  			$promotags_total = $this->model_catalog_promotags->getTotalProductsByPromoTagsId($promotags_id);
    
			if ($promotags_total) {
	  			$this->error['warning'] = sprintf($this->language->get('error_promotags'), $promotags_total);	
			}	
	  	} 
