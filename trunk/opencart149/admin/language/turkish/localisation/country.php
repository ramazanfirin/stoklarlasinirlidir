<?php
/* OpenCart 1.4.9.5 Turkish Ceviren eka7a - http://www.opencart-tr.com */

// Heading
$_['heading_title']          = 'Ülkeler';

// Text
$_['text_success']           = 'Başarılı: Ülkeler başarılı bir şekilde değiştirildi!';

// Column
$_['column_name']            = 'Ülke Adı';
$_['column_iso_code_2']      = 'ISO Kodu (2)';
$_['column_iso_code_3']      = 'ISO Kodu (3)';
$_['column_action']          = 'Aksiyon';

// Entry
$_['entry_status']           = 'Ülke Durumu:';
$_['entry_name']             = 'Ülke Adı:';
$_['entry_iso_code_2']       = 'ISO Kodu (2):';
$_['entry_iso_code_3']       = 'ISO Kodu (3):';
$_['entry_address_format']   = 'Adres Formatı:<br /><span class="help">
Ad = {firstname}<br />
Soyad = {lastname}<br />
Şirket = {company}<br />
Adres 1 = {address_1}<br />
Adres 2 = {address_2}<br />
İlçe / Semt = {city}<br />
Posta Kodu = {postcode}<br />
Şehir = {zone}<br />
Şehir Kodu = {zone_code}<br />
Ülke = {country}</span>';
$_['entry_postcode_required']= 'Posta Kodu Gerekli:';

// Error
$_['error_permission']       = 'Uyarı: Ülkeleri düzenleme iznine sahip değilsiniz!';
$_['error_name']             = 'Ülke Adı 3 ile 128 karakter arasında olmalı!';
$_['error_default']          = 'Uyarı: Bu ülkeyi silemezsiniz. Bu ükle varsayılan mağaza tarafından kullanılıyor!';
$_['error_store']          	 = 'Uyarı: Bu ülkeyi silemezsiniz. Bu ükle %s mağaza tarafından kullanılıyor!';
$_['error_address']          = 'Uyarı: Bu ülkeyi silemezsiniz. Bu ükle %s adres defterinde kullanılıyor!';
$_['error_zone']             = 'Uyarı: Bu ülkeyi silemezsiniz. Bu ülke %s şehirinde kullanılıyor!';
$_['error_zone_to_geo_zone'] = 'Uyarı: Bu ülkeyi silemezsiniz. Bu ülke %s genel bölgesinde kullanılıyor!';
?>